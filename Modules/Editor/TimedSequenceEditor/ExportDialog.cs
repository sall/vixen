	...
	public partial class ExportDialog : Form
	{
		...
		private void SequenceNotify(Vixen.Export.ExportNotifyType notifyType)
        {
            ...
            switch(notifyType)
            {
				...
				case ExportNotifyType.UPLOAD_SEQUENCE:
				{
					ItemUploadingSequence();
					break;
				}
				case ExportNotifyType.UPLOAD_OGG:
				{
					ItemUploadingOGG();
					break;
				}
				case ExportNotifyType.UPLOAD_MP3:
				{
					ItemUploadingMP3();
					break;
				}
				case ExportNotifyType.UPLOAD_UNIVERSES:
				{
					ItemUploadingUniverse();
					break;
				}
		...
		private void ItemUploadingUniverse()
		{
			if (this.InvokeRequired)
			{

				this.BeginInvoke(new Action(ItemUploadingUniverse));
				return;
			}
			else
			{
				setWorkingState("Uploading Universes: ", true, false);
			}
		}

		private void ItemUploadingSequence()
		{
			if (this.InvokeRequired)
			{

				this.BeginInvoke(new Action(ItemUploadingSequence));
				return;
			}
			else
			{
				setWorkingState("Uploading Sequence: ", true, false);
			}
		}
		private void ItemUploadingMP3( )
		{
			if (this.InvokeRequired)
			{

				this.BeginInvoke(new Action(ItemUploadingMP3));
				return;
			}
			else
			{
				setWorkingState( "Uploading MP3: " , true, false);
			}
		}

		private void ItemUploadingOGG()
		{
			if (this.InvokeRequired)
			{

				this.BeginInvoke(new Action(ItemUploadingOGG));
				return;
			}
			else
			{
				setWorkingState("Uploading OGG: ", true, false);
			}
		}
